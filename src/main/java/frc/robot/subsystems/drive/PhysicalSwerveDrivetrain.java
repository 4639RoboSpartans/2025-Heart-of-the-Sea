...
public class PhysicalSwerveDrivetrain extends AbstractSwerveDrivetrain {
    ...
    private Rotation2d getDirectionOfMovement(double vx, double vy) {
        return Rotation2d.fromRadians(Math.atan(vy / vx)
                + (!(vx < 0) && (vy < 0) ? Math.PI : 0)
                + (DriverStationUtil.getAlliance() == Alliance.Blue ? Math.PI : 0)
        );
    }
    ...
    public void followPath(SwerveSample sample) {
        ...
        double sampleSpeed = Math.hypot(sample.vx, sample.vy);
        double rawKp = 6 - sampleSpeed;
        double newKp = rawKp * 3 / 6;
        SmartDashboard.putNumber("Path kP", newKp);
        pathXController.setP(newKp);
        pathYController.setP(newKp);
        SmartDashboard.putNumber("Sample Speed", sampleSpeed);
        ...
        targetSpeeds = ChassisSpeeds.fromRobotRelativeSpeeds(
                adjustedRobotSpeeds,
                getPose().getRotation()
        );
        ...
        robotSpeeds = ChassisSpeeds.fromFieldRelativeSpeeds(targetSpeeds, getPose().getRotation());
    ...
    public void periodic() {
        ...
        SmartDashboard.putNumber("Left LC", Subsystems.lasercanAlign().getLeftMeasurement().orElse(Double.NaN));
        SmartDashboard.putNumber("Right LC", Subsystems.lasercanAlign().getRightMeasurement().orElse(Double.NaN));
    ...
    public Optional<Rotation2d> getCalculatedRotationFromAlign() {
        OptionalDouble leftMeasurement = Subsystems.lasercanAlign().getLeftMeasurement();
        OptionalDouble rightMeasurement = Subsystems.lasercanAlign().getRightMeasurement();

        if (leftMeasurement.isEmpty() || rightMeasurement.isEmpty()) {
            return Optional.empty();
        }

        double measurementDifference = leftMeasurement.getAsDouble() - rightMeasurement.getAsDouble();
        double measuredAngleRadians = Math.tan(
            measurementDifference / DriveConstants.laserCanDistanceMM.in(Millimeters)
        );
        return Optional.of(
